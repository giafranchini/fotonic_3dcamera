...
// sprintf_s(m_szDeviceDescription, sizeof(m_szDeviceDescription), "%s [%s]", m_aDeviceInfo[iDevice].szShortName, m_aDeviceInfo[iDevice].szSerial);
  sprintf(
    m_szDeviceDescription, "%s [%s]", m_aDeviceInfo[iDevice].szShortName,
    m_aDeviceInfo[iDevice].szSerial);
...
bool C_FZCamera::SetActive(char * szDevicePath)
{
  ...
  iResultIoctl |= FZ_IOCtl(
    ...
    NULL, NULL, NULL);
  FZ_IOCtl(
    m_hDevice,              // ignore result since old cameras dont have this command
  ...
  FZ_IOCtl(
    m_hDevice, CMD_DE_SET_MS_SATURATION, &m_iMSSaturation, sizeof(m_iMSSaturation), NULL,
    NULL, NULL);
...
bool C_FZCamera::SetMode(unsigned short iMode)
{
  ...
  FZ_Result iResult = FZ_IOCtl(
    m_hDevice, CMD_DE_SET_MODE, &m_iMode, sizeof(m_iMode), &iRespCode,
    NULL, NULL);
...
bool C_FZCamera::SetShutter(unsigned short iShutterMs10)
{
  ...
  FZ_Result iResult = FZ_IOCtl(
    m_hDevice, CMD_DE_SET_SHUTTER, &m_iShutterMs10,
    sizeof(m_iShutterMs10), &iRespCode, NULL, NULL);
...
bool C_FZCamera::SetFrameRate(unsigned short iFPS, unsigned short iFPSDiv)
{
  ...
  if (iOldFPS != iFPS) {
    iResult = FZ_IOCtl(m_hDevice, CMD_DE_SET_FPS, &m_iFPS, sizeof(m_iFPS), &iRespCode, NULL, NULL);
  }
  //set divisor but ignore result since old cameras dont have this command
  if (iOldFPSDiv != iFPSDiv) {
    FZ_IOCtl(
      m_hDevice, CMD_DE_SET_FPS_DIVISOR, &m_iFPSDiv, sizeof(m_iFPSDiv), &iRespCode, NULL,
      NULL);
  }
...
bool C_FZCamera::SetMSShutter2(unsigned short iShutterMs10)
{
  ...
  FZ_Result iResult = FZ_IOCtl(
    m_hDevice, CMD_DE_SET_SHUTTER_EXT, &stShutters, sizeof(stShutters),
    &iRespCode, NULL, NULL);
  return iResult == FZ_Success;
}
bool C_FZCamera::SetMSSaturation(unsigned short iSaturation)
{
  ...
  FZ_Result iResult =
    FZ_IOCtl(
    m_hDevice, CMD_DE_SET_MS_SATURATION, &m_iMSSaturation, sizeof(m_iMSSaturation),
    &iRespCode, NULL, NULL);
  return iResult == FZ_Success;
}
...
bool C_FZCamera::SetFiltering(
  unsigned short iLerpOn, unsigned short iEdgeOn, unsigned short iEdgeMinB,
  unsigned short iEdgeDiff1, unsigned short iEdgeDiff2, unsigned short iEdgeDiff3)
{
  ...
  iResult = FZ_IOCtl(
    m_hDevice, CMD_DE_SET_LERP_FILTER, &m_iLerpOn, sizeof(m_iLerpOn), &iRespCode,
    NULL, NULL);
  iResult |= FZ_IOCtl(
    m_hDevice, CMD_DE_SET_EDGE_FILTER, &m_iEdgeParams[0], sizeof(m_iEdgeParams),
    &iRespCode, NULL, NULL);
  return iResult == FZ_Success;
